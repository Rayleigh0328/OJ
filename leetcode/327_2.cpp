...
void print(RbTree<long long> &rb_tree){
    RbTree<long long>::Node* p = rb_tree.get_min_node();
    while (p!=NULL){
        cout << p->data << " ";
        p = rb_tree.get_successor(p);
    }
    cout << endl;
    
    p = rb_tree.get_min_node();
    while (p!=NULL){
        cout << p->size << " ";
        p = rb_tree.get_successor(p);
    }
...
public:
    int countRangeSum(vector<int>& a, int lower, int upper) {
        int ans = 0;
        long long sum = 0ll;
        RbTree<long long> sum_set;
        sum_set.insert(0ll);
        for (auto e : a){
            sum += e;
            ans += sum_set.count_less_than(sum - lower + 1) - sum_set.count_less_than(sum - upper);
            sum_set.insert(sum);
        }
        return ans;
    }
